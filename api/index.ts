...
const missingVars = !process.env.DATABASE_URL || !process.env.SESSION_SECRET;

// If env vars are missing, return a clear JSON error instead of crashing with an HTML page
if (missingVars) {
  app.use((_req: any, res: any) => {
    res.status(500).json({
      error: "Configuração incompleta. Adicione DATABASE_URL e SESSION_SECRET nas variáveis de ambiente do Vercel e faça Redeploy.",
    });
  });
} else {
  // Dynamic imports so the DB module only loads when DATABASE_URL is present
  const { db } = await import("../server/db.js");
  const { empreendimentos, clientes, vendas, appConfig, localUsers } = await import("../shared/schema.js");
  const { gerarContratoParceladoPadrao } = await import("../server/contratoParceladoPadrao.js");
...
app.get("/api/empreendimentos", isAuthenticated, async (req: any, res) => {
  ...
  } catch (e: any) { res.status(500).json({ error: "Failed to fetch empreendimentos" }); }
});
...
app.post("/api/empreendimentos", isAuthenticated, async (req: any, res) => {
  try {
    ...
    for (const id of existingIds)
      if (!newIds.has(id)) await db.delete(empreendimentos).where(and(eq(empreendimentos.id, id as string), eq(empreendimentos.userId, userId)));
    ...
    res.json({ ok: true });
  } catch (e: any) { res.status(500).json({ error: "Failed to save empreendimentos" }); }
});
...
app.get("/api/clientes", isAuthenticated, async (req: any, res) => {
  try {
    ...
    res.json(rows.map((r: any) => r.data));
  } catch (e: any) { res.status(500).json({ error: "Failed to fetch clientes" }); }
});
...
app.post("/api/clientes", isAuthenticated, async (req: any, res) => {
  try {
    ...
    const newIds = new Set(items.map((e: any) => e.id));
    for (const id of existingIds)
      if (!newIds.has(id)) await db.delete(clientes).where(and(eq(clientes.id, id as string), eq(clientes.userId, userId)));
    for (const item of items)
      await db.insert(clientes).values({ id: item.id, userId, data: item }).onConflictDoUpdate({ target: clientes.id, set: { data: item } });
    res.json({ ok: true });
  } catch (e: any) { res.status(500).json({ error: "Failed to save clientes" }); }
});
...
app.get("/api/vendas", isAuthenticated, async (req: any, res) => {
  try {
    ...
    res.json(rows.map((r: any) => r.data));
  } catch (e: any) { res.status(500).json({ error: "Failed to fetch vendas" }); }
});
...
app.post("/api/vendas", isAuthenticated, async (req: any, res) => {
  try {
    ...
    const newIds = new Set(items.map((e: any) => e.id));
    for (const id of existingIds)
      if (!newIds.has(id)) await db.delete(vendas).where(and(eq(vendas.id, id as string), eq(vendas.userId, userId)));
    for (const item of items)
      await db.insert(vendas).values({ id: item.id, userId, data: item }).onConflictDoUpdate({ target: vendas.id, set: { data: item } });
    res.json({ ok: true });
  } catch (e: any) { res.status(500).json({ error: "Failed to save vendas" }); }
});
...
app.get("/api/config", isAuthenticated, async (req: any, res) => {
  ...
  } catch (e: any) { res.status(500).json({ error: "Failed to fetch config" }); }
});
...
app.post("/api/config", isAuthenticated, async (req: any, res) => {
  try {
    ...
    res.json({ ok: true });
  } catch (e: any) { res.status(500).json({ error: "Failed to save config" }); }
});
...
app.post("/api/gemini/extract-files", isAuthenticated, async (req, res) => {
  try {
    ...
    const response = await fetch(GEMINI_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ contents: [{ role: "user", parts }] }) });
    const data = await response.json() as any;
    res.json(safeParseJson(data.candidates?.[0]?.content?.parts?.[0]?.text ?? "{}"));
  } catch (err: any) { res.status(500).json({ error: String(err?.message || err) }); }
});
...
app.post("/api/gemini/analyze-map", isAuthenticated, async (req, res) => {
  try {
    ...
    const response = await fetch(GEMINI_URL, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ contents: [{ role: "user", parts: [{ inlineData: { mimeType, data: base64Data } }, { text: `Analise este mapa de loteamento e extraia as informações de lotes, quadras e ruas. Retorne APENAS JSON puro (sem markdown): {"lotes":[{"quadra":"A","lote":"01","rua":"Nome da Rua"}],"totalLotes":0,"ruasEncontradas":["Rua 1"]}` }] }] }),
    });
    const data = await response.json() as any;
    res.json(safeParseJson(data.candidates?.[0]?.content?.parts?.[0]?.text ?? "{}"));
  } catch (err: any) { res.status(500).json({ error: String(err?.message || err) }); }
...
app.post("/api/contrato/parcelado-padrao", isAuthenticated, async (req, res) => {
  try {
    ...
    res.setHeader("Content-Disposition", `attachment; filename="contrato_${nomeCliente}_${nomeEmp}.docx"`);
