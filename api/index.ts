...
if (!process.env.DATABASE_URL || !process.env.SESSION_SECRET) {
  // Return a clear JSON error instead of crashing with HTML "A server error occurred"
  app.use((_req: any, res: any) => {
    res.status(500).json({
      error:
        "Configuração incompleta no Vercel. Adicione DATABASE_URL e SESSION_SECRET nas variáveis de ambiente e faça Redeploy.",
    });
  });
} else {
    ...
    try {
      ...
      if (!email || !password || password.length < 6)
        return res.status(400).json({
          error: "E-mail e senha (mínimo 6 caracteres) são obrigatórios.",
        });
  ...
  app.get("/api/empreendimentos", isAuthenticated, async (req: any, res) => {
    try {
      const rows = await db
        ...
        .where(eq(empreendimentos.userId, getUserId(req)));
  ...
  app.post("/api/empreendimentos", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const existing = await db
        ...
        .from(empreendimentos)
        .where(eq(empreendimentos.userId, userId));
      ...
      for (const id of existingIds)
        if (!newIds.has(id))
          await db
            ...
            .where(
              and(
                eq(empreendimentos.id, id as string),
                eq(empreendimentos.userId, userId)
              )
            );
      for (const item of items)
        await db
          .insert(empreendimentos)
          .values({ id: item.id, userId, data: item })
          .onConflictDoUpdate({
            target: empreendimentos.id,
            set: { data: item },
          });
      res.json({ ok: true });
    } catch {
  ...
  app.get("/api/clientes", isAuthenticated, async (req: any, res) => {
    try {
      const rows = await db
        .select()
        .from(clientes)
        .where(eq(clientes.userId, getUserId(req)));
      res.json(rows.map((r: any) => r.data));
    } catch {
  ...
  app.get("/api/vendas", isAuthenticated, async (req: any, res) => {
    try {
      const rows = await db
        .select()
        .from(vendas)
        .where(eq(vendas.userId, getUserId(req)));
      res.json(rows.map((r: any) => r.data));
    } catch {
      ...
      for (const id of existingIds)
        if (!newIds.has(id))
          await db
            .delete(vendas)
            .where(
              and(eq(vendas.id, id as string), eq(vendas.userId, userId))
            );
      for (const item of items)
        await db
          .insert(vendas)
  ...
  app.get("/api/config", isAuthenticated, async (req: any, res) => {
    try {
      const [row] = await db
        .select()
        .from(appConfig)
        .where(eq(appConfig.userId, getUserId(req)));
  ...
  app.post("/api/gemini/extract-sale", isAuthenticated, async (req, res) => {
    try {
      ...
      res.json(
        safeParseJson(data.candidates?.[0]?.content?.parts?.[0]?.text ?? "{}")
      );
    } catch (err: any) {
  ...
  app.post("/api/gemini/extract-files", isAuthenticated, async (req, res) => {
    try {
      ...
      const parts: any[] = files.map((f: any) => ({
        inlineData: { mimeType: f.mimeType, data: f.base64 },
      }));
      parts.push({
        text: `Extraia os dados dos documentos e responda SOMENTE em JSON puro, sem markdown, no formato: {"nomeComprador":"","cpf":"","rg":"","nascimento":"YYYY-MM-DD ou vazio","estadoCivil":"","profissao":"","nacionalidade":"","endereco":"","numero":"","bairro":"","cidade":"","estado":"","cep":"","telefone1":"","telefone2":"","numeroLote":"","quadra":"","valorLote":null,"valorEntrada":null,"quantidadeParcelas":null,"valorParcela":null,"dataVencimento":"YYYY-MM-DD ou vazio","vendedor":""}. Campos não encontrados: "" ou null.`,
      });
      const response = await fetch(GEMINI_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    async (req, res) => {
      try {
        ...
        if (!vendedor || !cliente || !empreendimento || !venda)
          return res
            .status(400)
            .json({ error: "Dados incompletos para gerar o contrato." });
        const buffer = await gerarContratoParceladoPadrao({
          vendedor,
          cliente,
          empreendimento,
          venda,
        ...
        const nomeEmp = (empreendimento.nome as string)
          .replace(/\s+/g, "_")
          .toUpperCase();
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="contrato_${nomeCliente}_${nomeEmp}.docx"`
        );
        res.setHeader(
          "Content-Type",
