...
// Debug endpoint — helps diagnose missing env vars on Vercel
...
if (missingVars.length > 0) {
  app.use((_req: any, res: any) => {
    res.status(500).json({
      error: `Variáveis de ambiente ausentes: ${missingVars.join(", ")}. Adicione em Settings → Environment Variables no Vercel e faça Redeploy.`,
    });
  });
} else {
    ...
    try {
      ...
      if (!match)
        return res
          .status(401)
          .json({ error: "E-mail ou senha incorretos." });
      (req.session as any).localUser = { id: user.id, email: user.email, isAdmin: user.isAdmin };
  ...
  app.get("/api/auth/user", isAuthenticated, async (req: any, res) => {
    ...
    try {
      const [row] = await db.select({ isAdmin: localUsers.isAdmin }).from(localUsers).where(eq(localUsers.id, u.id));
      res.json({ id: u.id, email: u.email, isAdmin: row?.isAdmin ?? false });
    } catch {
  ...
  app.get("/api/admin/users", isAuthenticated, async (req: any, res) => {
    try {
      const u = (req.session as any)?.localUser;
      const [self] = await db.select({ isAdmin: localUsers.isAdmin }).from(localUsers).where(eq(localUsers.id, u.id));
      if (!self?.isAdmin) return res.status(403).json({ error: "Acesso restrito." });
  ...
  app.delete("/api/admin/users/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      await db.delete(localUsers).where(eq(localUsers.id, req.params.id));
      res.json({ ok: true });
    } catch (e: any) {
