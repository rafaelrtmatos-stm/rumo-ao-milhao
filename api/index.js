...
import pg2 from "pg";
import { eq as eq2, and, ne } from "drizzle-orm";
...
app.delete("/api/admin/users/:id", isAuthenticated, isAdminUser, async (req, res) => {
  try {
    const requesterId = req.tokenUser?.id || req.session?.localUser?.id;
    if (req.params.id === requesterId) return res.status(400).json({ error: "Voc\xEA n\xE3o pode excluir sua pr\xF3pria conta." });
...
app.post("/api/admin/migrate-to-shared", isAuthenticated, isAdminUser, async (_req, res) => {
  try {
    const SHARED = "shared";
    ...
    for (const row of oldDevs) {
      ...
      await db.delete(empreendimentos).where(and(eq2(empreendimentos.id, row.id), ne(empreendimentos.userId, SHARED)));
    ...
    for (const row of oldClients) {
      ...
      await db.delete(clientes).where(and(eq2(clientes.id, row.id), ne(clientes.userId, SHARED)));
      moved.clientes++;
    }
    ...
    for (const row of oldVendas) {
      ...
      await db.delete(vendas).where(and(eq2(vendas.id, row.id), ne(vendas.userId, SHARED)));
      moved.vendas++;
    }
    const sharedConfig = await db.select().from(appConfig).where(eq2(appConfig.userId, SHARED));
    ...
    res.status(500).json({ error: e?.message || "Erro na migra\xE7\xE3o." });
  }
});
...
app.put("/api/vendas/:id", isAuthenticated, async (req, res) => {
  res.setHeader("Cache-Control", "no-store");
  try {
    const item = req.body;
    if (!item || !req.params.id) return res.status(400).json({ error: "Dados inv\xE1lidos." });
    await db.insert(vendas).values({ id: req.params.id, userId: SHARED_USER, data: item }).onConflictDoUpdate({ target: vendas.id, set: { data: item } });
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e?.message || "Failed to upsert venda" });
  }
});
app.delete("/api/vendas/:id", isAuthenticated, async (req, res) => {
  res.setHeader("Cache-Control", "no-store");
  try {
    await db.delete(vendas).where(and(eq2(vendas.id, req.params.id), eq2(vendas.userId, SHARED_USER)));
    res.json({ ok: true });
  } catch (e) {
...
app.put("/api/clientes/:id", isAuthenticated, async (req, res) => {
  ...
  try {
    const item = req.body;
    if (!item || !req.params.id) return res.status(400).json({ error: "Dados inv\xE1lidos." });
    await db.insert(clientes).values({ id: req.params.id, userId: SHARED_USER, data: item }).onConflictDoUpdate({ target: clientes.id, set: { data: item } });
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e?.message || "Failed to upsert cliente" });
  }
});
app.put("/api/empreendimentos/:id", isAuthenticated, async (req, res) => {
  res.setHeader("Cache-Control", "no-store");
  try {
    const item = req.body;
    if (!item || !req.params.id) return res.status(400).json({ error: "Dados inv\xE1lidos." });
    await db.insert(empreendimentos).values({ id: req.params.id, userId: SHARED_USER, data: item }).onConflictDoUpdate({ target: empreendimentos.id, set: { data: item } });
    res.json({ ok: true });
  } catch (e) {
