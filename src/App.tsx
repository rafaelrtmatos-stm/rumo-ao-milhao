...
const exportToCSV = (sales: Venda[]) => {
  const headers = ["Contrato", "Cliente", "Empreendimento", "Quadra", "Lote", "Valor", "Status", "Data"];
  ...
  link.setAttribute("download", `vendas_rumo_ao_milhao_${new Date().toLocaleDateString()}.csv`);
...
const Sidebar = ({ currentSection, setSection, isOpen, setIsOpen, onLogout }: { currentSection: Section, setSection: (s: Section) => void, isOpen: boolean, setIsOpen: (val: boolean) => void, onLogout?: () => void }) => {
  ...
  return (
    <>
      ...
      <div className={`w-72 bg-surface-card h-screen flex flex-col fixed left-0 top-0 border-r border-border-subtle shadow-xl z-[60] transition-transform duration-300 transform ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="p-8 flex justify-between items-center bg-surface-card border-b border-border-subtle">
          <div>
            <h1 className="text-2xl font-display font-bold text-primary-main italic tracking-tight">Rumo ao Milhão</h1>
            <p className="text-[10px] uppercase font-bold text-slate-400 mt-0.5 tracking-widest">Soluções Imobiliárias</p>
          </div>
          <button onClick={() => setIsOpen(false)} className="lg:hidden p-2 text-slate-400 hover:text-slate-600 rounded-xl hover:bg-slate-50">
            <X size={20} />
          </button>
        </div>
...
const Header = ({ title, toggleSidebar }: { title: string, toggleSidebar: () => void }) => (
  <header className="h-20 lg:h-24 bg-surface-card/80 backdrop-blur-md border-b border-border-subtle flex items-center px-6 lg:px-10 fixed top-0 right-0 left-0 lg:left-72 z-40">
    <button onClick={toggleSidebar} className="lg:hidden p-3 mr-4 bg-surface-bg hover:bg-slate-100 rounded-2xl text-slate-600 transition-colors">
      <LayoutDashboard size={22} />
    </button>
    <h2 className="text-xl lg:text-2xl font-display font-bold text-slate-800 tracking-tight truncate">{title}</h2>
  </header>
);

const BottomNav = ({ currentSection, setSection }: { currentSection: Section, setSection: (s: Section) => void }) => {
  const items = [
      ...
      {items.map((item) => {
        ...
        return (
          ...
          >
            ...
            <Icon size={isActive ? 22 : 20} className={isActive ? 'stroke-[2.5]' : 'stroke-[2]'} />
  ...
  <div className={`stat-card-gradient ${colorClass}`}>
    <div className="flex justify-between items-start">
      <div>
        ...
        <p className="text-4xl font-display font-bold tracking-tight">{value}</p>
      </div>
...
const DashboardSection = ({ sales, developments }: { sales: Venda[], developments: Empreendimento[] }) => {
  const totalRevenue = sales.reduce((acc, sale) => acc + sale.valorLote, 0);
  const totalCommissions = sales.reduce((acc, sale) => acc + (sale.comissao || 0), 0);
  ...
  const salesData = last7Days.map(day => ({
    name: day,
    total: sales.filter(s => new Date(s.dataVenda).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' }) === day).length
  }));
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        <StatCard 
          title="Comissões" 
          value={new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 }).format(totalCommissions)} 
          icon={Calculator} 
          colorClass="bg-gradient-to-br from-chumbo-base to-chumbo-muted text-primary-contrast" 
        />
        <motion.div whileHover={{ y: -5 }} className="card-premium border-primary-main/20 bg-primary-main/[0.02]">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-slate-400 mb-2">Lucro Líquido</p>
              <p className={`text-3xl font-display font-bold tracking-tight ${totalProfit >= 0 ? 'text-success-main' : 'text-red-500'}`}>
                {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 }).format(totalProfit)}
              </p>
            </div>
            <div className="p-3 bg-surface-card shadow-sm border border-border-subtle rounded-2xl">
              <TrendingUp size={24} className={totalProfit >= 0 ? 'text-success-main' : 'text-red-500'} />
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="card-premium">
          <div className="flex items-center gap-2 mb-8">
            ...
            <h4 className="font-display font-bold text-slate-800">Tendência de Vendas (7 dias)</h4>
          </div>
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={salesData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                ...
                />
                <Bar dataKey="total" fill="#2d5016" radius={[6, 6, 0, 0]} barSize={24} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="card-premium">
          <div className="flex items-center gap-2 mb-8">
            ...
            <h4 className="font-display font-bold text-slate-800">Ocupação por Loteamento</h4>
          </div>
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={occupancyData} layout="vertical" margin={{ top: 0, right: 30, left: 40, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#f1f5f9" />
                <XAxis type="number" hide />
                <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fontSize: 10, fontWeight: 700, fill: '#1e293b' }} />
                <Tooltip 
                  cursor={{ fill: 'transparent' }}
                  contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' }}
                />
                <Bar dataKey="total" fill="#e2e8f0" radius={[0, 4, 4, 0]} barSize={12} />
                <Bar dataKey="vendidos" fill="#2d5016" radius={[0, 4, 4, 0]} barSize={12} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="card-premium">
        <div className="flex items-center justify-between mb-8">
          ...
          <button className="text-sm font-bold text-primary-main hover:underline">Ver tudo</button>
        </div>
            ...
            <tbody className="text-sm">
              {sales.slice(0, 5).map((venda) => (
                <tr key={venda.id} className="group">
                  <td className="py-4 px-4 bg-slate-50 group-hover:bg-primary-main/5 rounded-l-2xl transition-colors font-semibold">{venda.clienteNome}</td>
                  <td className="py-4 px-4 bg-slate-50 group-hover:bg-primary-main/5 transition-colors text-slate-500">{venda.empreendimentoNome}</td>
                  <td className="py-4 px-4 bg-slate-50 group-hover:bg-primary-main/5 transition-colors font-mono font-bold text-xs">L:{venda.numeroLote}/Q:{venda.quadra}</td>
              ...
              {sales.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-12 text-center text-slate-300 font-medium italic">Nenhuma venda registrada ainda.</td>
                </tr>
              )}
            </tbody>
          </table>
...
const LotDashboard = ({ dev, sales, onStartSale, onClose }: { dev: Empreendimento, sales: Venda[], onStartSale: (v: Partial<Venda>) => void, onClose: () => void }) => {
  ...
  const isSold = (q: string, l: string) => {
    return soldLots.some(s => s.quadra.toUpperCase() === q.toUpperCase() && s.numeroLote === l);
  };

  const getSale = (q: string, l: string) => {
    return soldLots.find(s => s.quadra.toUpperCase() === q.toUpperCase() && s.numeroLote === l);
  };
      ...
      >
        <div className="p-6 sm:p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-display font-bold text-slate-800">{dev.nome}</h3>
              <p className="text-sm text-slate-400 font-medium">Dashboard de Ocupação de Lotes</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-10 space-y-12">
          {quadras.length > 0 ? (
            quadras.map(q => {
              ...
              const displayLots = lotsInQuadra.length > 0 ? lotsInQuadra.sort((a,b) => Number(a) - Number(b)) : Array.from({length: 12}, (_, i) => (i + 1).toString());

              return (
                <div key={q} className="space-y-4">
                  <div className="flex items-center gap-3">
                    <h4 className="px-4 py-1.5 bg-slate-900 text-white rounded-lg font-display font-bold text-sm">Quadra {q}</h4>
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3">
                    {displayLots.map(l => {
                      ...
                      const sold = !!soldData;
                      const lotInfo = dev.lotesInfo?.[`${q}-${l}`.toUpperCase()];
                      ...
                      return (
                        ...
                        >
                          ...
                          {sold ? (
                            <div className="mt-2 p-1 bg-red-100 rounded-full text-[8px] font-bold uppercase tracking-widest">Vendido</div>
                          ) : (
                            <button 
                              onClick={() => onStartSale({ empreendimentoId: dev.id, quadra: q, numeroLote: l, rua: lotInfo?.rua })}
                          ...
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-3 bg-slate-900 text-white text-[10px] rounded-xl opacity-0 group-hover:opacity-100 pointer-events-none transition-all z-10 shadow-xl">
                            <p className="font-bold border-b border-white/10 pb-1.5 mb-1.5 uppercase tracking-widest">Detalhes do Lote</p>
                            <p><span className="text-white/40">Status:</span> {sold ? 'Vendido' : 'Disponível'}</p>
                            {lotInfo?.rua && <p><span className="text-white/40">Rua:</span> {lotInfo.rua}</p>}
                            {soldData && <p><span className="text-white/40">Cliente:</span> {soldData.clienteNome}</p>}
            ...
            <div className="text-center py-20 space-y-4">
              ...
              <p className="text-slate-400 font-medium">Nenhuma quadra cadastrada para este empreendimento.</p>
              <p className="text-xs text-slate-300 max-w-xs mx-auto">Tente ler o mapa com a IA para identificar as quadras e lotes automaticamente.</p>
            </div>
          )}
        </div>
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-center gap-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Vendido</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-primary-main" />
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Selecionado</span>
          </div>
        </div>
      </motion.div>
    </div>
...
const EmpreendimentosSection = ({ developments, sales, onSave, onDelete, onUpdateLotesInfo, onStartSale }: { developments: Empreendimento[], sales: Venda[], onSave: (d: Empreendimento) => void, onDelete: (id: string) => void, onUpdateLotesInfo: (id: string, info: Record<string, { rua: string }>) => void, onStartSale: (v: Partial<Venda>) => void }) => {
  const [isAdding, setIsAdding] = useState(false);
  const [formData, setFormData] = useState<Partial<Empreendimento>>({
    nome: '', endereco: '', cidade: '', estado: '', totalLotes: 0, descricao: '',
    comunidade: '', quadras: '', ruas: ''
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({ ...formData as Empreendimento, id: Date.now().toString(), lotesVendidos: 0, lotesInfo: {} });
    setIsAdding(false);
    setFormData({ nome: '', endereco: '', cidade: '', estado: '', totalLotes: 0, descricao: '', comunidade: '', quadras: '', ruas: '' });
  };
  ...
  const handleMapUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setFormData(prev => ({
        ...
        ruas: result.ruasEncontradas ? result.ruasEncontradas.join(', ') : prev.ruas,
        lotesInfo: newLotesInfo
      }));

      alert(`Sucesso! IA identificou ${result.lotes.length} lotes e ${result.ruasEncontradas?.length || 0} ruas no mapa.`);
      setIsAnalyzing(false);
    } catch (err) {
      console.error(err);
      alert("Erro ao analisar mapa. Verifique se a API Key do Gemini está configurada corretamente.");
      setIsAnalyzing(false);
    }
  };
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xl font-display font-bold text-slate-800">Empreendimentos</h3>
            <p className="text-sm text-slate-400 font-medium">Gestão de Loteamentos</p>
          </div>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          <label className="btn-ghost flex-1 sm:flex-none relative overflow-hidden cursor-pointer">
            <input type="file" className="hidden" accept="image/*,application/pdf" onChange={handleMapUpload} disabled={isAnalyzing} />
            {isAnalyzing ? (
          ...
          <button onClick={() => setIsAdding(!isAdding)} className="btn-primary flex-1 sm:flex-none">
      ...
      <AnimatePresence>
        {isAdding && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
            <form onSubmit={handleSubmit} className="card-premium grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50/50">
              ...
              <div className="md:col-span-1">
                ...
                <input className="input-field" value={formData.cidade} onChange={e => setFormData({...formData, cidade: e.target.value})} placeholder="0" />
              </div>
              <div className="md:col-span-1">
                <label className="label">Estado</label>
                <input className="input-field font-bold" value={formData.estado} onChange={e => setFormData({...formData, estado: e.target.value})} placeholder="0" />
              </div>

              <div className="md:col-span-2">
                <label className="label">Localização / Bairro Geral</label>
                <input className="input-field" value={formData.endereco} onChange={e => setFormData({...formData, endereco: e.target.value})} placeholder="Rua / Travessa" />
              </div>
              <div>
                <label className="label">Comunidade / Região</label>
                <input className="input-field" value={formData.comunidade} onChange={e => setFormData({...formData, comunidade: e.target.value})} placeholder="Ex: Centro, Vila Nova" />
              </div>
              <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label className="label">Quadras Disponíveis</label>
                  <input className="input-field" value={formData.quadras} onChange={e => setFormData({...formData, quadras: e.target.value})} placeholder="Ex: A, B, C, D" />
                </div>
                <div>
                  <label className="label">Ruas / Acessos</label>
                  <input className="input-field" value={formData.ruas} onChange={e => setFormData({...formData, ruas: e.target.value})} placeholder="Ex: Rua 01, Rua 02, Av. Principal" />
                </div>
              </div>
              <div>
                <label className="label">Total de Lotes</label>
                <input type="number" className="input-field font-bold" value={formData.totalLotes} onChange={e => setFormData({...formData, totalLotes: Number(e.target.value)})} />
              </div>
              <div className="md:col-span-2">
                <label className="label">Destaque de Venda</label>
                <input className="input-field" value={formData.descricao} onChange={e => setFormData({...formData, descricao: e.target.value})} placeholder="Ex: Vista para o lago, Próximo ao centro" />
                <p className="text-[10px] text-slate-400 font-bold uppercase mt-2 ml-1 opacity-60 flex items-center gap-1">
                  ...
                  Nota: Quadras e Ruas podem ser separadas por vírgula para organização.
                </p>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {developments.map(dev => (
          ...
          >
            ...
            <div className="mb-6 flex items-center gap-4">
              ...
              <div>
                <h4 className="text-xl font-display font-bold text-slate-800 leading-tight">{dev.nome}</h4>
            ...
            <div className="mb-4 px-1">
              ...
              {dev.quadras && (
                <p className="text-xs text-slate-500 line-clamp-1">
                  <span className="font-bold text-slate-700">Quadras:</span> {dev.quadras}
                </p>
              )}
            </div>
            ...
            <div className="mt-auto space-y-5 bg-slate-50/50 p-5 rounded-2xl border border-slate-100">
              <div className="flex justify-between items-end">
                <div className="space-y-1">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">Vendas</p>
                  <p className="text-2xl font-display font-bold text-slate-800 leading-none">
                    {dev.lotesVendidos} <span className="text-sm font-medium text-slate-400">/ {dev.totalLotes}</span>
                  </p>
                </div>
              ...
              <div className="relative">
                <div className="w-full bg-slate-200 h-2.5 rounded-full overflow-hidden">
                  <motion.div 
                    initial={{ width: 0 }}
                    animate={{ width: `${(dev.lotesVendidos / dev.totalLotes) * 100}%` }}
              ...
              <div className="flex justify-between items-center text-[11px] font-bold uppercase tracking-widest pt-2 border-t border-slate-100">
                ...
                <span className="text-primary-main">{Object.keys(dev.lotesInfo || {}).length} lotes mapeados</span>
              </div>
        ...
        {developments.length === 0 && !isAdding && (
          <div className="col-span-full py-20 text-center flex flex-col items-center gap-4 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
            ...
            <p className="text-slate-400 font-medium italic">Sua base de empreendimentos está vazia.</p>
            <button onClick={() => setIsAdding(true)} className="btn-ghost text-sm mt-2 font-bold px-8">
...
const VendasSection = ({ developments, onSaveVenda, onGoToContracts, initialSaleData }: { developments: Empreendimento[], onSaveVenda: (v: Venda, c: Cliente) => Venda, onGoToContracts: (v: Venda) => void, initialSaleData?: Partial<Venda> }) => {
  const [clientData, setClientData] = useState<Partial<Cliente>>({
    nome: '', nacionalidade: 'Brasileira', genero: 'M', rg: '', cpf: '', estadoCivil: 'Solteiro(a)', profissao: '', nascimento: '',
    cep: '', endereco: '', numero: '', bairro: '', cidade: '', estado: '', telefone1: '', telefone2: ''
  });
  ...
  const [saleData, setSaleData] = useState<Partial<Venda>>({
    empreendimentoId: '', numeroLote: '', quadra: '', rua: '', valorLote: undefined,
    valorEntrada: undefined, quantidadeParcelas: undefined, dataVencimento: '', vendedor: '',
    valorParcela: 0, custo: 0, comissao: 0, formaPagamento: 'Boleto',
    ...initialSaleData
  });
...
Vendedor: ${lastSavedVenda.vendedor}
  ...
  }, [saleData.empreendimentoId, saleData.quadra, saleData.numeroLote, developments]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const dev = developments.find(d => d.id === saleData.empreendimentoId);
    const cliente: Cliente = { ...clientData as Cliente, id: Date.now().toString(), dataCadastro: new Date().toISOString() };
    const venda: Venda = {
      ...saleData as Venda,
  ...
  if (lastSavedVenda) {
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="card-premium py-20 text-center space-y-8">
        ...
        <div className="space-y-2">
          <h2 className="text-3xl font-display font-bold text-slate-800">Venda Registrada!</h2>
          <p className="text-slate-500">O cadastro foi concluído e os dados estão salvos.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center pt-8">
          ...
          <button onClick={() => onGoToContracts(lastSavedVenda)} className="btn-primary px-8">
            <FileText size={18} />
            <span>Gerar Contrato Agora</span>
          </button>
          <button onClick={() => setLastSavedVenda(null)} className="btn-ghost px-8">
    ...
    <div className="space-y-8 pb-32 lg:pb-0">
      ...
      <div className="card-premium bg-gradient-to-br from-primary-main/[0.03] to-transparent border-primary-main/10">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-lg font-display font-bold text-slate-800">Auto-preenchimento Inteligente</h3>
            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Cole informações brutas aqui</p>
          </div>
        </div>
      ...
      <div className="card-premium">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h3 className="text-lg font-display font-bold text-slate-800">Dados do Proponente</h3>
            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest leading-none mt-1">Identificação e Contato</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2 flex flex-col sm:flex-row gap-4">
            <div className="flex-1">
              ...
              <input required className="input-field" value={clientData.nome} onChange={e => setClientData({...clientData, nome: e.target.value})} placeholder="Nome Completo" />
            </div>
              ...
              <div className="flex gap-2 p-1 bg-slate-100 rounded-2xl">
                <button 
                  ...
                  onClick={() => {
                    ...
                    if (newEstadoCivil === 'Solteira') newEstadoCivil = 'Solteiro';
                    else if (newEstadoCivil === 'Casada') newEstadoCivil = 'Casado';
                    else if (newEstadoCivil === 'Divorciada') newEstadoCivil = 'Divorciado';
                    else if (newEstadoCivil === 'Viúva') newEstadoCivil = 'Viúvo';
                    setClientData({...clientData, genero: newGenero, estadoCivil: newEstadoCivil});
                  }}
                ...
                <button 
                  ...
                  onClick={() => {
                    const newGenero = 'F';
                    let newEstadoCivil = clientData.estadoCivil;
                    if (newEstadoCivil === 'Solteiro') newEstadoCivil = 'Solteira';
                    else if (newEstadoCivil === 'Casado') newEstadoCivil = 'Casada';
                    else if (newEstadoCivil === 'Divorciado') newEstadoCivil = 'Divorciada';
                    else if (newEstadoCivil === 'Viúvo') newEstadoCivil = 'Viúva';
                    setClientData({...clientData, genero: newGenero, estadoCivil: newEstadoCivil});
                  }}
                ...
                <button 
                  type="button"
                  onClick={() => setClientData({...clientData, genero: 'O'})}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
          <div>
            ...
            <input required className="input-field font-mono" value={clientData.cpf} onChange={e => setClientData({...clientData, cpf: maskCPF(e.target.value)})} placeholder="000.000.000-00" />
          </div>
          <div>
            <label className="label">RG</label>
            <input required className="input-field font-mono" value={clientData.rg} onChange={e => setClientData({...clientData, rg: e.target.value})} placeholder="00.000.000-0" />
          </div>
          <div>
            <label className="label">Estado Civil</label>
            <select className="input-field font-semibold" value={clientData.estadoCivil} onChange={e => setClientData({...clientData, estadoCivil: e.target.value})}>
              <option>{clientData.genero === 'F' ? 'Solteira' : 'Solteiro'}</option>
              <option>{clientData.genero === 'F' ? 'Casada' : 'Casado'}</option>
              <option>{clientData.genero === 'F' ? 'Divorciada' : 'Divorciado'}</option>
          ...
          <div>
            ...
            <input type="date" required className="input-field font-semibold" value={clientData.nascimento} onChange={e => setClientData({...clientData, nascimento: e.target.value})} />
          </div>
          <div>
            <label className="label">CEP</label>
            <input required className="input-field font-mono" value={clientData.cep} onChange={e => { 
          ...
          <div className="md:col-span-1">
            ...
            <input required className="input-field" value={clientData.endereco} onChange={e => setClientData({...clientData, endereco: e.target.value})} placeholder="Rua / Travessa" />
          </div>
          <div>
            <label className="label">Nº</label>
            <input required className="input-field font-bold" value={clientData.numero} onChange={e => setClientData({...clientData, numero: e.target.value})} placeholder="0" />
          </div>
          <div>
            <label className="label">Bairro</label>
            <input required className="input-field" value={clientData.bairro} onChange={e => setClientData({...clientData, bairro: e.target.value})} placeholder="Bairro" />
          </div>
          <div className="hidden">
            <label className="label">Cidade / UF (Auto)</label>
            <div className="flex gap-2">
              <input className="input-field flex-1" value={clientData.cidade} readOnly />
              <input className="input-field w-16 text-center font-bold" value={clientData.estado} readOnly />
            </div>
          </div>
          <div>
            <label className="label">Contato Principal</label>
            <input required className="input-field font-semibold" value={clientData.telefone1} onChange={e => setClientData({...clientData, telefone1: maskPhone(e.target.value)})} placeholder="(00) 00000-0000" />
          </div>
          <div>
            <label className="label">Contato Secundário</label>
            <input className="input-field" value={clientData.telefone2} onChange={e => setClientData({...clientData, telefone2: maskPhone(e.target.value)})} placeholder="(00) 00000-0000" />
          </div>
        </div>
        ...
        <div className="mt-8 pt-8 border-t border-slate-100">
           <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h4 className="font-bold text-slate-700">Segundo Comprador (Adicional)</h4>
              </div>
              <button 
                type="button"
           ...
           {hasSecondBuyer && (
             <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-6 overflow-hidden">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2 flex flex-col sm:flex-row gap-4">
                    <div className="flex-1">
                      ...
                      <input required className="input-field" value={secondBuyerData?.nome} onChange={e => setSecondBuyerData({...secondBuyerData!, nome: e.target.value})} placeholder="Nome Completo" />
                    </div>
                    <div className="w-full sm:w-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pb-4">
                  <div>
                    <label className="label">CPF</label>
                    <input required className="input-field font-mono" value={secondBuyerData?.cpf} onChange={e => setSecondBuyerData({...secondBuyerData!, cpf: maskCPF(e.target.value)})} placeholder="000.000.000-00" />
                  </div>
                  <div>
                    <label className="label">RG</label>
                    <input required className="input-field font-mono" value={secondBuyerData?.rg} onChange={e => setSecondBuyerData({...secondBuyerData!, rg: e.target.value})} placeholder="00.000.000-0" />
                  </div>
                  <div>
                    <label className="label">Nacionalidade</label>
                    <input required className="input-field" value={secondBuyerData?.nacionalidade} onChange={e => setSecondBuyerData({...secondBuyerData!, nacionalidade: e.target.value})} placeholder="Brasileira" />
                  </div>
                  <div>
                    <label className="label">Estado Civil</label>
                    <select className="input-field font-semibold" value={secondBuyerData?.estadoCivil} onChange={e => setSecondBuyerData({...secondBuyerData!, estadoCivil: e.target.value})}>
                      <option>{secondBuyerData?.genero === 'F' ? 'Solteira' : 'Solteiro'}</option>
                      <option>{secondBuyerData?.genero === 'F' ? 'Casada' : 'Casado'}</option>
                      <option>{secondBuyerData?.genero === 'F' ? 'Divorciada' : 'Divorciado'}</option>
                      <option>{secondBuyerData?.genero === 'F' ? 'Viúva' : 'Viúvo'}</option>
                      <option>União Estável</option>
                    </select>
                  </div>
                  <div>
                    <label className="label">Aniversário</label>
                    <input type="date" required className="input-field" value={secondBuyerData?.nascimento} onChange={e => setSecondBuyerData({...secondBuyerData!, nascimento: e.target.value})} />
                  </div>
                </div>
             </motion.div>
           )}
      ...
      <div className="card-premium border-primary-light/10 bg-primary-light/[0.02]">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h3 className="text-lg font-display font-bold text-slate-800">Negociação e Pagamento</h3>
            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest leading-none mt-1">Dados Comerciais</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="sm:col-span-2 space-y-4">
            ...
            <select required className="input-field font-bold text-primary-main" value={saleData.empreendimentoId} onChange={e => setSaleData({...saleData, empreendimentoId: e.target.value})}>
              <option value="">Escolha um loteamento...</option>
              {developments.map(d => (
                <option key={d.id} value={d.id}>{d.nome} — {d.totalLotes - d.lotesVendidos} livres</option>
              ))}
            </select>
            
            {saleData.empreendimentoId && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="p-4 bg-surface-card border border-border-subtle rounded-2xl space-y-2">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Informações de Apoio</p>
                    <h4 className="font-bold text-slate-800">{developments.find(d => d.id === saleData.empreendimentoId)?.nome}</h4>
                  </div>
                  <div className="px-2 py-1 bg-primary-main/10 text-primary-main text-[10px] font-bold rounded-lg uppercase">
                    {developments.find(d => d.id === saleData.empreendimentoId)?.comunidade || 'Geral'}
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4 text-xs">
                  <p><span className="font-bold text-slate-500">Quadras:</span> {developments.find(d => d.id === saleData.empreendimentoId)?.quadras || 'Não informada'}</p>
                  <p><span className="font-bold text-slate-500">Ruas:</span> {developments.find(d => d.id === saleData.empreendimentoId)?.ruas || 'Não informada'}</p>
                </div>
              </motion.div>
            )}
          </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:col-span-2">
              <div>
                ...
                <input required className="input-field font-mono font-bold" value={saleData.numeroLote} onChange={e => setSaleData({...saleData, numeroLote: e.target.value})} placeholder="0" />
              </div>
              <div>
                <label className="label">Quadra</label>
                <input required list="quadras-list" className="input-field font-mono font-bold" value={saleData.quadra} onChange={e => setSaleData({...saleData, quadra: e.target.value})} placeholder="A" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <label className="label font-bold text-primary-main">Valor Total</label>
              <input type="number" step="0.01" className="input-field border-primary-main/20 bg-primary-main/[0.02] font-display font-bold text-lg" value={saleData.valorLote ?? ''} onChange={e => handleValueChange('valorLote', e.target.value === '' ? 0 : Number(e.target.value))} />
            </div>
            <div>
              <label className="label">Entrada</label>
              <input type="number" step="0.01" className="input-field font-display font-bold text-lg" value={saleData.valorEntrada ?? ''} onChange={e => handleValueChange('valorEntrada', e.target.value === '' ? 0 : Number(e.target.value))} />
            </div>
            <div>
              <label className="label">Quantidade de Parcelas</label>
              <input type="number" className="input-field font-bold text-lg" value={saleData.quantidadeParcelas ?? ''} onChange={e => handleValueChange('quantidadeParcelas', e.target.value === '' ? 1 : Number(e.target.value))} />
            </div>
            <div>
              <label className="label font-bold text-chumbo-base opacity-70">Valor da Parcela</label>
              <input type="number" step="0.01" className="input-field border-chumbo-base/20 bg-chumbo-base/[0.02] font-display font-bold text-lg" value={saleData.valorParcela || ''} onChange={e => handleValueChange('valorParcela', e.target.value === '' ? 0 : Number(e.target.value))} />
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="date" className="input-field pl-12 font-bold" value={saleData.dataVencimento} onChange={e => setSaleData({...saleData, dataVencimento: e.target.value})} />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            <div className="bg-surface-card/60 backdrop-blur-sm p-8 rounded-3xl border border-border-subtle flex-1 flex flex-col justify-center shadow-inner">
              <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-[0.2em] mb-4 text-center">Resumo da Negociação</p>
              
              <div className="space-y-6">
                <div className="flex justify-between items-center border-b border-slate-50 pb-4">
                  <span className="text-sm font-medium text-slate-500">Valor da Mensalidade</span>
                  <p className="text-3xl font-display font-bold text-primary-main">
                    {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(saleData.valorParcela || 0)}
                  </p>
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">Saldo Financiado</p>
                    <p className="font-display font-bold text-slate-700">
                      {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format((saleData.valorLote || 0) - (saleData.valorEntrada || 0))}
                    </p>
                  </div>
                  <div className="space-y-1 text-right">
                    <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">Total Líquido</p>
                    <p className="font-display font-bold text-slate-700">
                      {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(saleData.valorLote || 0)}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <button type="submit" className="btn-primary w-full shadow-lg shadow-primary-main/20">
...
const ContratosSection = ({ sales, clients, developments, initialVenda, onUpdateStatus }: { sales: Venda[], clients: Cliente[], developments: Empreendimento[], initialVenda?: Venda | null, onUpdateStatus: (id: string, s: 'pendente' | 'pago' | 'cancelado') => void }) => {
  const [selectedVenda, setSelectedVenda] = useState<Venda | null>(initialVenda || null);
  ...
  const client = selectedVenda ? clients.find(c => c.id === selectedVenda.clienteId) : null;
  const development = selectedVenda ? developments.find(d => d.id === selectedVenda.empreendimentoId) : null;
  ...
  const getStatusInfo = (status?: string) => {
    switch (status) {
      case 'pago': return { label: 'Pago', color: 'text-success-main bg-success-main/10', icon: CheckCircle2 };
      case 'cancelado': return { label: 'Cancelado', color: 'text-red-500 bg-red-50', icon: AlertCircle };
      default: return { label: 'Pendente', color: 'text-amber-500 bg-amber-50', icon: Clock };
    }
  };
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 px-2">
        <div className="space-y-1">
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{sales.length} emitidos</p>
        </div>
              ...
              {filteredSales.map((venda, idx) => {
                ...
                return (
                  <tr key={venda.id} className="group">
                    ...
                    <td className="py-4 px-4 bg-slate-50 group-hover:bg-primary-main/5 transition-colors font-semibold text-slate-700">{venda.clienteNome}</td>
                    <td className="py-4 px-4 bg-slate-50 group-hover:bg-primary-main/5 transition-colors text-slate-500">{venda.empreendimentoNome}</td>
                    <td className="py-4 px-4 bg-slate-50 group-hover:bg-primary-main/5 transition-colors text-right font-display font-bold text-primary-main">
                      {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(venda.valorLote)}
                    </td>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 lg:p-8 bg-slate-900/40 backdrop-blur-md no-print">
            ...
            >
              <div className="p-6 lg:p-8 border-b border-border-subtle flex justify-between items-center bg-surface-card">
                <div className="flex items-center gap-4">
                  <div className="p-3 bg-primary-main rounded-2xl text-primary-contrast">
                    {viewMode === 'contract' ? <FileText size={24} /> : <FileCheck size={24} />}
                  </div>
                  <div>
                    <h4 className="font-display font-bold text-slate-800 text-lg">
                      {viewMode === 'contract' ? 'Visualização de Contrato' : 'Visualização de Recibo'}
                    </h4>
                    <div className="flex items-center gap-2">
                       <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{selectedVenda.numeroContrato}</p>
                ...
                <div className="flex gap-3">
                  <div className="hidden sm:flex self-center items-center gap-4 mr-4 border-r border-slate-200 pr-4">
                    <button 
                      onClick={() => setViewMode(viewMode === 'contract' ? 'receipt' : 'contract')}
                    ...
                    >
                      Mudar para {viewMode === 'contract' ? 'Recibo' : 'Contrato'}
                    </button>
                  </div>
                  <button onClick={handlePrint} className="btn-primary h-12 px-6">
                    <Printer size={18} /> <span className="hidden sm:inline">Imprimir</span>
                  </button>
                  <button onClick={() => setSelectedVenda(null)} className="h-12 w-12 flex items-center justify-center text-slate-400 hover:bg-slate-50 rounded-2xl transition-colors">
                    <X size={24} />
                  </button>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-8 lg:p-12 bg-slate-100/50">
                {viewMode === 'contract' ? (
                  <div id="contract-content" className="bg-white shadow-2xl p-8 sm:p-20 mx-auto w-full max-w-[21cm] min-h-[29.7cm] text-black text-[13px] leading-[1.8] font-serif border border-slate-200">
                    <div className="text-center space-y-3 mb-16">
                      <h1 className="text-2xl font-bold underline underline-offset-8 decoration-1 mb-8 uppercase">Instrumento Particular de Promessa de Compra e Venda</h1>
                      <div className="flex justify-center gap-12 font-mono text-[10px] text-slate-400">
                        ...
                        <span>DATA: {new Date(selectedVenda.dataVenda).toLocaleDateString('pt-BR')}</span>
                      </div>
                    </div>

                    <section className="mb-10">
                      <h2 className="font-bold mb-4 uppercase tracking-wider border-b border-slate-800 py-1 text-sm bg-slate-50 px-2 italic">1. QUALIFICAÇÃO DAS PARTES</h2>
                      <div className="px-2 space-y-4 leading-relaxed text-justify">
                        <p>
                          <span className="font-bold">PROMITENTE VENDEDOR:</span> <span className="uppercase font-bold">GENILSON PEREIRA MOREIRA</span>, brasileiro, solteiro, portador do CPF nº 632.939.002-91, residente e domiciliado em Santarém/PA.
                        </p>
                        <p>
                          <span className="font-bold">
                            {client?.genero === 'F' ? 'PROMISSÁRIA COMPRADORA' : client?.genero === 'O' ? 'PROMISSÁRIO(A) COMPRADOR(A)' : 'PROMISSÁRIO COMPRADOR'}:
                          </span> <span className="font-bold uppercase underline">{client?.nome}</span>, {client?.genero === 'F' ? 'brasileira' : client?.genero === 'O' ? 'brasileiro(a)' : 'brasileiro'}, {client?.estadoCivil.toLowerCase()}, {client?.profissao ? client.profissao + ', ' : ''}{client?.genero === 'F' ? 'portadora' : client?.genero === 'O' ? 'portador(a)' : 'portador'} da carteira de identidade nº {client?.rg} e do CPF nº {client?.cpf}, {client?.genero === 'F' ? 'residente e domiciliada' : client?.genero === 'O' ? 'residente e domiciliado(a)' : 'residente e domiciliado'} na {client?.endereco}, nº {client?.numero}, {client?.bairro}, {client?.cidade}/{client?.estado}, CEP {client?.cep}.
                        </p>
                        {selectedVenda.comprador2 && (
                          <p>
                            <span className="font-bold">
                              {selectedVenda.comprador2.genero === 'F' ? 'SEGUNDA PROMISSÁRIA' : selectedVenda.comprador2.genero === 'O' ? 'SEGUNDO(A) PROMISSÁRIO(A)' : 'SEGUNDO PROMISSÁRIO'}:
                            </span> <span className="font-bold uppercase underline">{selectedVenda.comprador2.nome}</span>, {selectedVenda.comprador2.genero === 'F' ? 'brasileira' : selectedVenda.comprador2.genero === 'O' ? 'brasileiro(a)' : 'brasileiro'}, {selectedVenda.comprador2.estadoCivil.toLowerCase()}, {selectedVenda.comprador2.profissao ? selectedVenda.comprador2.profissao + ', ' : ''}{selectedVenda.comprador2.genero === 'F' ? 'portadora' : selectedVenda.comprador2.genero === 'O' ? 'portador(a)' : 'portador'} da carteira de identidade nº {selectedVenda.comprador2.rg} e do CPF nº {selectedVenda.comprador2.cpf}.
                          </p>
                        )}
                      </div>
                    </section>
                    ...
                    <section className="mb-10">
                      <h2 className="font-bold mb-4 uppercase tracking-wider border-b border-slate-800 py-1 text-sm bg-slate-50 px-2 italic">2. OBJETO DO NEGÓCIO</h2>
                      <p className="px-2 leading-relaxed text-justify">
                        Constitui objeto deste contrato a alienação do <span className="font-bold text-lg underline">LOTE Nº {selectedVenda.numeroLote}</span>, 
                        integrante da <span className="font-bold">QUADRA {selectedVenda.quadra}</span>, 
                        situado na <span className="font-bold font-mono tracking-tight underline">{selectedVenda.rua || '___________________'}</span>, 
                        no empreendimento <span className="font-bold uppercase italic underline decoration-slate-300">{selectedVenda.empreendimentoNome}</span> 
                        {development?.comunidade ? `, localizado na comunidade/região ${development.comunidade}` : ''}
                        {development?.ruas ? `, próximo à(s) rua(s) ${development.ruas}` : ''}. O referido imóvel faz parte do projeto aprovado pelos órgãos competentes.
                      </p>
                    </section>

                    <section className="mb-12">
                      <h2 className="font-bold mb-4 uppercase tracking-wider border-b border-slate-800 py-1 text-sm bg-slate-50 px-2 italic">3. DO VALOR E FORMA DE PAGAMENTO</h2>
                      <div className="bg-slate-50 p-6 rounded-2xl space-y-4 border border-slate-100">
                        <p>O preço certo e ajustado da venda ora prometido é de <span className="font-bold text-xl text-primary-dark">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(selectedVenda.valorLote)}</span>, que será pago via <span className="font-bold underline">{selectedVenda.formaPagamento}</span> da seguinte forma:</p>
                        
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div className="p-4 border border-slate-200 bg-white rounded-xl shadow-sm">
                            <p className="text-[10px] text-slate-500 font-bold uppercase mb-1">Sinal / Entrada</p>
                            <p className="font-bold text-black text-lg">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(selectedVenda.valorEntrada)}</p>
                            <p className="text-[9px] text-slate-400 mt-1 font-bold">PAGO NO ATO DA ASSINATURA</p>
                          </div>
                          <div className="p-4 border border-slate-200 bg-white rounded-xl shadow-sm">
                            <p className="text-[10px] text-slate-500 font-bold uppercase mb-1">Saldo Remanescente</p>
                            <p className="font-bold text-black text-lg">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(selectedVenda.valorLote - selectedVenda.valorEntrada)}</p>
                            <p className="text-[9px] text-slate-400 mt-1 font-bold">FINANCIAMENTO PRÓPRIO</p>
                          </div>
                        </div>

                        <p className="text-sm font-medium leading-relaxed">
                          O saldo remanescente será quitado em <span className="font-bold">{selectedVenda.quantidadeParcelas} parcelas fixas</span>, 
                          no valor de <span className="font-bold underline">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(selectedVenda.valorParcela)}</span> cada uma, 
                          com vencimento no dia <span className="font-bold underline">{new Date(selectedVenda.dataVencimento).getDate()}</span> de cada mês, 
                          ficando a primeira parcela para o dia <span className="font-bold underline">{new Date(selectedVenda.dataVencimento).toLocaleDateString('pt-BR')}</span>.
                        </p>
                      </div>
                    </section>
                    ...
                    <section className="mt-32">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-16 px-10">
                        <div className="text-center">
                          ...
                          <p className="text-[10px] font-bold uppercase text-slate-400 mb-1">Representante Vendedor</p>
                          <p className="font-bold text-slate-800 tracking-tight">{selectedVenda.vendedor || 'GENILSON P. MOREIRA'}</p>
                        </div>
                        <div className="text-center">
                          <div className="h-px bg-slate-900 mb-2" />
                          <p className="text-[10px] font-bold uppercase text-slate-400 mb-1">Promitente Comprador</p>
                          <p className="font-bold text-slate-800 tracking-tight text-sm uppercase">{selectedVenda.clienteNome}</p>
                        </div>
                      </div>
                      <div className="text-center mt-20 text-[9px] text-slate-300 font-bold tracking-widest uppercase">
                        Santarém/PA, {new Date(selectedVenda.dataVenda).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}
                      </div>
                    </section>
                  </div>
                ) : (
                  <div className="bg-white shadow-2xl p-8 sm:p-20 mx-auto w-full max-w-[21cm] min-h-[15cm] text-black font-sans border border-slate-200">
                    ...
                    <div className="space-y-8 text-lg leading-loose">
                      <p className="text-justify">
                        Recebemos de <span className="font-bold uppercase underline underline-offset-4">{selectedVenda.clienteNome}</span>, 
                        inscrito(a) no CPF nº <span className="font-bold">{client?.cpf}</span>, 
                        a importância supra de <span className="font-bold italic">({new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(selectedVenda.valorEntrada)})</span>, 
                        referente ao <span className="font-bold">SINAL E PRINCÍPIO DE PAGAMENTO (ENTRADA)</span> para aquisição do imóvel:
                      </p>
                    ...
                    <div className="mt-20 pt-10 border-t border-slate-100 flex justify-between items-end">
                       <div>
                          <p className="text-sm font-bold text-slate-800">Santarém/PA, {new Date(selectedVenda.dataVenda).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}</p>
                       </div>
                       <div className="w-64 text-center">
                          <div className="h-px bg-slate-900 mb-2" />
                          <p className="text-[10px] font-bold uppercase text-slate-400">Assinatura do Vendedor</p>
                          <p className="font-bold text-slate-900">{selectedVenda.vendedor || 'GENILSON P. MOREIRA'}</p>
                       </div>
                    </div>
                  </div>
                )}
...
const ClientesSection = ({ clients, sales }: { clients: Cliente[], sales: Venda[] }) => {
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center px-2">
        <h3 className="text-xl font-display font-bold text-slate-800 flex items-center gap-3">
        ...
        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{clients.length} cadastrados</p>
      </div>
        ...
        <div className="overflow-x-auto -mx-2 sm:mx-0">
          <table className="w-full text-left border-separate border-spacing-y-1 sm:border-spacing-y-2">
            ...
            <tbody className="text-sm">
                ...
                return (
                  <tr key={cliente.id} className="group">
                    <td className="py-2.5 px-2 sm:px-4 bg-slate-50 group-hover:bg-primary-main/5 rounded-l-xl sm:rounded-l-2xl transition-colors">
                      ...
                      <div className="text-[7px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-tighter sm:tracking-normal">{cliente.estadoCivil}</div>
                    </td>
                    <td className="py-2.5 px-2 sm:px-4 bg-slate-50 group-hover:bg-primary-main/5 transition-colors font-mono text-xs text-slate-500 hidden sm:table-cell">{cliente.cpf}</td>
                    <td className="py-2.5 px-2 sm:px-4 bg-slate-50 group-hover:bg-primary-main/5 transition-colors">
                      <div className="text-[10px] sm:text-xs font-bold text-slate-600">{cliente.telefone1}</div>
                    </td>
              ...
              {clients.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-16 text-center text-slate-300 italic font-medium">Nenhum cliente na base.</td>
                </tr>
              )}
            </tbody>
          </table>
...
const AniversariosSection = ({ clients }: { clients: Cliente[] }) => {
  ...
  const proximos90Dias = clients.filter(c => {
    const bday = new Date(c.nascimento);
    ...
    <div className="space-y-12">
      <section>
        <div className="flex items-center gap-3 mb-6 px-2">
          ...
          <h3 className="text-xl font-display font-bold text-slate-800 tracking-tight">Celebrando este Mês</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {meshAniversariantes.map(cliente => (
            <motion.div whileHover={{ y: -5 }} key={cliente.id} className="card-premium bg-gradient-to-br from-chumbo-light/20 to-white flex gap-5 items-center border-chumbo-base/20">
              <div className="p-4 bg-chumbo-base text-primary-contrast rounded-2xl shadow-lg shadow-chumbo-base/30">
                <Cake size={28} />
              </div>
              <div>
                <p className="font-display font-bold text-slate-800 text-lg leading-tight">{cliente.nome}</p>
                <div className="flex items-center gap-2 text-xs font-bold text-chumbo-base uppercase tracking-widest mt-1">
                  <span>{new Date(cliente.nascimento).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long' })}</span>
                  <span className="opacity-30">•</span>
                  <span className="text-slate-400 font-mono">{cliente.telefone1}</span>
                </div>
              </div>
      ...
      <section>
        <div className="flex items-center gap-3 mb-6 px-2">
          ...
          <h3 className="text-xl font-display font-bold text-slate-800 tracking-tight">Próximos 90 dias</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {proximos90Dias.map(cliente => (
            <div key={cliente.id} className="card-premium flex gap-5 items-center opacity-80 hover:opacity-100 transition-all">
              <div className="p-3.5 bg-slate-50 text-slate-300 rounded-2xl font-mono text-center leading-none">
                <p className="text-[10px] uppercase font-bold text-slate-400">{new Date(cliente.nascimento).toLocaleDateString('pt-BR', { month: 'short' })}</p>
                <p className="text-lg font-bold text-slate-600">{new Date(cliente.nascimento).getDate()}</p>
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-bold text-slate-700 truncate">{cliente.nome}</p>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{cliente.telefone1}</p>
              </div>
              <a 
          ...
          {proximos90Dias.length === 0 && (
            <p className="col-span-full text-center text-slate-300 py-4 font-medium italic">Sem aniversariantes no radar.</p>
          )}
        </div>
      </section>
    </div>
...
const ConfigSection = ({ config, onSave }: { config: AppConfig, onSave: (c: AppConfig) => void }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      <div className="flex items-center gap-3 px-2">
        ...
        <h3 className="text-xl font-display font-bold text-slate-800 tracking-tight">Configurações Gerais</h3>
      </div>
            ...
            {themes.map(theme => (
              <button
                ...
                className={`p-4 rounded-2xl border-2 transition-all text-left flex flex-col gap-3 ${
                  formData.theme === theme.id ? 'border-primary-main bg-primary-main/5' : 'border-slate-100 bg-slate-50'
                }`}
              >
                <div className={`w-10 h-10 rounded-xl ${theme.color} shadow-sm`} />
                <span className={`font-bold text-xs uppercase tracking-widest ${formData.theme === theme.id ? 'text-primary-main' : 'text-slate-400'}`}>
                  {theme.label}
                </span>
              </button>
            ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="sm:col-span-2">
                  <label className="label font-bold text-primary-main">Valor Total do Lote (R$)</label>
                  <input type="number" className="input-field border-primary-main/20 bg-primary-main/[0.02] text-2xl font-display font-bold text-primary-dark" value={data.valor ?? ''} onChange={e => handleCalcChange('valor', e.target.value === '' ? 0 : Number(e.target.value))} />
                </div>
                <div>
                  <label className="label">Entrada (R$)</label>
                  <input type="number" className="input-field font-bold text-lg" value={data.entrada ?? ''} onChange={e => handleCalcChange('entrada', e.target.value === '' ? 0 : Number(e.target.value))} />
                </div>
                <div>
                  <label className="label">Qtd. Parcelas</label>
                  <input type="number" className="input-field font-bold text-lg" value={data.parcelas ?? ''} onChange={e => handleCalcChange('parcelas', e.target.value === '' ? 1 : Number(e.target.value))} />
                </div>
                <div className="sm:col-span-2">
                  <label className="label font-bold text-chumbo-base opacity-70">Valor Desejado da Parcela (R$)</label>
                  <input type="number" step="0.01" className="input-field border-chumbo-base/20 bg-chumbo-base/[0.02] text-2xl font-display font-bold text-slate-800" value={data.parcelaValue ?? ''} onChange={e => handleCalcChange('parcelaValue', e.target.value === '' ? 0 : Number(e.target.value))} />
                  <p className="text-[10px] text-slate-400 font-bold uppercase mt-2 ml-1 italic opacity-60">Alterar o valor da parcela recalcula o valor total automaticamente</p>
                </div>
              </div>
              ...
              <div className="pt-6 border-t border-slate-100">
                ...
                <div className="flex items-center gap-4">
                  <input type="range" min="1" max="15" step="0.5" className="flex-1 accent-primary-main" value={data.comissaoPercent} onChange={e => setData({...data, comissaoPercent: Number(e.target.value)})} />
                  <span className="w-16 text-center font-display font-bold text-primary-main bg-primary-main/10 py-2 rounded-xl">{data.comissaoPercent}%</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-12 xl:col-span-5">
          <div className="bg-slate-900 text-white rounded-[40px] p-10 shadow-[0_30px_100px_-20px_rgba(15,23,42,0.3)] relative overflow-hidden group border border-white/5">
            ...
            <h3 className="text-[10px] font-extrabold uppercase tracking-[0.3em] text-primary-light opacity-60 mb-10">Resumo da Proposta</h3>
            
            <div className="space-y-8 relative">
              <div className="space-y-1">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest opacity-60">Total do Lote</p>
                <p className="text-4xl font-display font-bold tracking-tight">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(data.valor || 0)}</p>
              </div>

              <div className="grid grid-cols-2 gap-8">
                <div className="space-y-1">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest opacity-60">Entrada</p>
                  <p className="text-xl font-display font-bold text-primary-light">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(data.entrada || 0)}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest opacity-60">Saldo</p>
                  <p className="text-xl font-display font-bold text-white/90">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format((data.valor || 0) - (data.entrada || 0))}</p>
                </div>
              </div>

              <div className="pt-10 border-t border-white/10">
                <p className="text-[10px] font-extrabold uppercase text-primary-light/60 tracking-widest mb-3">Mensalidade Recomendada</p>
                <div className="flex items-baseline gap-3">
                  <p className="text-6xl font-display font-bold text-white tracking-tighter drop-shadow-2xl">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(data.parcelaValue || 0)}</p>
                  <span className="text-sm font-medium text-white/30">/ mês</span>
                </div>
              </div>
              ...
              <div className="mt-12 p-8 bg-white/5 rounded-[32px] border border-white/10 backdrop-blur-md shadow-inner">
                <div className="flex justify-between items-center">
                  <div>
                    <p className="text-[10px] font-bold text-primary-light uppercase tracking-widest leading-none mb-1">Estimativa de Ganhos</p>
                    <p className="text-xs font-medium text-white/40 italic">Comissão fixa de {data.comissaoPercent}%</p>
                  </div>
          ...
          <button onClick={() => window.print()} className="w-full mt-6 btn-ghost !border-slate-200 !text-slate-400 hover:!text-slate-600 group">
...
export default function App({ onLogout }: { onLogout?: () => void }) {
  ...
  const renderSection = () => {
    switch (section) {
      case 'dashboard': return <DashboardSection sales={sales} developments={developments} />;
      case 'empreendimentos': return <EmpreendimentosSection developments={developments} sales={sales} onSave={saveDev} onDelete={deleteDev} onUpdateLotesInfo={updateLotesInfo} onStartSale={handleStartSale} />;
      case 'vendas': return <VendasSection developments={developments} onSaveVenda={saveSale} onGoToContracts={handleGoToContracts} initialSaleData={prefilledSale} />;
      case 'contratos': return <ContratosSection sales={sales} clients={clients} developments={developments} initialVenda={contractToOpen} onUpdateStatus={updateVendaStatus} />;
      case 'clientes': return <ClientesSection clients={clients} sales={sales} />;
      case 'aniversarios': return <AniversariosSection clients={clients} />;
      case 'calculadora': return <CalculatorSection />;
      case 'config': return <ConfigSection config={config} onSave={saveAppConfig} />;
      default: return <DashboardSection sales={sales} developments={developments} />;
    }
  };
